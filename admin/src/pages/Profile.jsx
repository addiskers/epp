    ...
    <div className="stack">
      ...
      <div className="card" style={{ maxWidth: 460 }}>
        <div className="stack" style={{ gap: 14 }}>
          ...
          <div><label>Role</label><span className={`pill ${isAdmin ? 'green' : 'blue'}`}>{isAdmin ? 'Admin' : 'Department user'}</span></div>
          {!isAdmin && <div><label>Department</label><div>{user?.department_name || '—'}</div></div>}
        </div>
      </div>
      <div className="card" style={{ maxWidth: 460 }}>
        ...
        <div className="row"><label>New password</label><input type="password" value={nw} onChange={(e) => setNw(e.target.value)} placeholder="min 8 characters" /></div>
